        ...
        <div class="container">
            <div class="row">
                <div class="col-sm-12 center-align">
                    <h1 >Reporte Ajuste de Inventario</h1>
                </div>
            </div>
            <div class="row">
            ...
            <table-byte :set-table="dataTable" :filters="['name']">
                ...
                <table-row slot="table-row" slot-scope="{ item }">
                    ...
                    <table-cell v-if="item.tipo_movimiento  == '3'">{{ item.cantidad }}</table-cell>
                    <table-cell v-else></table-cell>
                    <table-cell v-if="item.tipo_movimiento == '4'">{{ item.cantidad }}</table-cell>
                    <table-cell v-else></table-cell>                    
...
export default {
    ...
    methods: {
        _query() {
            ...
            axios.post('intranet/ajuste_reporte', this.form)
            .then(res=>{

                if (res.data.data == "") {
                    swal("No existen ajustes para este producto.")
                }
